...
{
    public class NET_BrushStroke : RealtimeComponent<NCM_BrushStrokeModel>
    {
        [SerializeField]
        private BrushStrokeMesh _mesh;
        [SerializeField]
        private SOC_NetSpawner m_NetSpawnerSock;
        ...
        protected override void OnRealtimeModelReplaced(NCM_BrushStrokeModel previousModel, NCM_BrushStrokeModel currentModel)
        {
            // Clear Mesh
            _mesh.ClearRibbon();
            ...
            {
                // Replace ribbon mesh
                foreach (NCM_RibbonPointModel ribbonPoint in currentModel.ribbonPoints)
            ...
            // Inform the network
            m_NetSpawnerSock.AddBoardBrushStroke(this);
        }
        ...
        private void AddRibbonPoint(Vector3 position, Quaternion rotation)
        {
            ...
            NCM_RibbonPointModel ribbonPoint = new NCM_RibbonPointModel();
            ribbonPoint.position = position;
            ribbonPoint.rotation = rotation;
            model.ribbonPoints.Add(ribbonPoint);
        }

        private void RibbonPointAdded(RealtimeArray<NCM_RibbonPointModel> ribbonPoints, NCM_RibbonPointModel ribbonPoint, bool remote)
        {
